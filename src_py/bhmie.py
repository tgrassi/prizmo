...
# modified for LKRM, see original notes below
def bhmie(x, refrel, nang):
    ...
    if nmx > nmxx:
        print("ERROR: in bhmie nmx > nmxx=%f for |m|x=%f" % (nmxx, ymod))
        sys.exit()
    ...
    #     First do angles from 0 to 90
        pii = 0 + pi1    # 0+pi1 because we want a hard copy of the values
        tau = en*amu*pii-(en+1.)*pi0
        s1_1 += fn * (an*pii+bn*tau)
        s2_1 += fn * (an*tau+bn*pii)
    ...
    #    of the second part of s1 and s2 after the calculation
        p = -p
        s1_2 += fn * p * (an*pii-bn*tau)
        s2_2 += fn * p * (bn*pii-an*tau)
    ...
    #     from PI = pi_n , PI0 = pi_n-1
        pi1 = ((2.*en+1.)*amu*pii - (en+1.)*pi0) / en
        pi0 = 0+pii   # 0+pi because we want a hard copy of the values
...
# compute qabs using miepython (note complex refractive index is m = n - ik)
def bhmie_qabs(x, refrel):
